...
.DESCRIPTION
Converts table-based outputs created by Invoke-SqlQuery to CSV format with option to re-map property names,
trim trailing spaces from fixed CHAR(x) properties, replace $DBNull:Value with $null, and apply formatting
to numeric and datetime properties. Invoke-SqlQuery has a FileName switch that directly produces a CSV file
which is a better solution if a CSV is all you need. For outputting trace data as a CSV, this works well.
...
Convert the DataTable data that was captured in the 2nd trace entry (the trace log is zero-based) to CSV, rename
the columns emp_name and h_date and apply date formatting to the hire date.
.NOTES
Author: Mike Dumdei
#>
    [CmdletBinding()]
    ...
    process {
        ...
        } elseif ($null -ne $SqlTraceItem -and $SqlTraceItem.Data -is [DataTable]) {
