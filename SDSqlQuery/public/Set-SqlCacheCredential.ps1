...
.DESCRIPTION
    Credentials may be preset at the start of a script using a credential cache so that a script does not have to pass credentials to Invoke-SqlQuery for each call. This cmdlet adds a credential to the credential cache. Unless using contained databases, set a Server specific credential or a "global" credential. A "global" credential is set by specifying only the Credential parameter and is used for all connections that do not have a more specific overriding match.

    Credentials passed to Invoke-SqlQuery via that cmdlet's Credential parameter override any stored in the cache.
